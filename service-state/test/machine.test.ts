...
describe('the machine', () => {
    ...
    test('it reads the service before reporting anything', async () => {
        service.observed = 'ON'
        const machine = await createServiceStateMachine(service, events)
...
describe('the methods', () => {
    test('all four, admin-only by default, each answering the status', async () => {
        const service = controllable()
        const machine = await createServiceStateMachine(service, sink())
        const methods = serviceStateMethods(machine)
        ...
        expect(await methods['service.status'].handler(ctx, undefined)).toMatchObject({ state: 'OFF' })
        expect(await methods['service.start'].handler(ctx, undefined)).toMatchObject({ state: 'STARTING' })
        service.settle()
        await settled()
        expect(await methods['service.stop'].handler(ctx, undefined)).toMatchObject({ state: 'STOPPING' })
        service.settle()
        await settled()
        expect(await methods['service.refresh'].handler(ctx, undefined)).toMatchObject({ state: 'OFF' })
    })

    test('the auth requirement can be relaxed for a read-only console', async () => {
        const machine = await createServiceStateMachine(controllable(), sink())
